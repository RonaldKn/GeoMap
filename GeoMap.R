geomap <- function(map, file, area="Nederland", zoom=7, type="terrain"){
  ...
  if(nlevels(df.points$GroupID) == 1){
    ...
    ggmap(map) + geom_point(aes(x=Long, y=Lat), data=df.group1, color=color1, size=size1, alpha=alpha1) + geom_point(aes(x=Long, y=Lat), data=df.group2, color=color2, size=size2, alpha=alpha2)
    ...
    ggmap(map) + geom_point(aes(x=Long, y=Lat), data=df.group1, color=color1, size=size1, alpha=alpha1) + geom_point(aes(x=Long, y=Lat), data=df.group2, color=color2, size=size2, alpha=alpha2) + geom_point(aes(x=Long, y=Lat), data=df.group3, color=color3, size=size3, alpha=alpha3)
  }else{
